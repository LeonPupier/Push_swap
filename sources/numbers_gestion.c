...
/*   Updated: 2022/12/22 13:54:14 by lpupier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	recover_list_index(t_data *data)
{
	...
	count = 0;
	data->list_index = malloc(sizeof(int) * data->len_stack);
	if (data->list_index == NULL)
		return (0);
	while (count < data->len_stack)
	{
		x = 0;
		y = 0;
		while (y < data->len_stack)
		{
			if (data->stack_a[y] < data->stack_a[count])
				x++;
			y++;
		}
